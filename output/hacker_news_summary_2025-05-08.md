...
本文强调了平方误差优于绝对误差的原因，因为它提供了平滑、连续的导数，使优化过程更容易且更可靠。最终，梯度下降使我们能够有效地找到最适合数据的最佳直线，从而实现准确的预测，例如估计房价。梯度下降的原理可以扩展到更复杂的模型，例如神经网络。
...
**原文链接**: [https://nexo.sh/posts/microservices-for-startups/](https://nexo.sh/posts/microservices-for-startups/)
...
本文探讨了华特迪士尼幻想工程公司即将推出的景点“华特·迪士尼—奇妙的一生”，该景点将推出一个先进的华特·迪士尼动画人偶，计划在迪士尼乐园70周年庆典上首次亮相。其目标是捕捉迪士尼的精髓，创造一个栩栩如生的形象，但该项目引发了伦理辩论，并受到了迪士尼家族褒贬不一的反应。
...
该笔记本电脑包含WPS（微软Office替代品）和阿里巴巴钉钉等中国软件。它还兼容鸿蒙智能手机上越来越多的移动应用程序，如赤焰笔记、哔哩哔哩和飞书，并计划在年底前支持2000多个应用程序。此举标志着华为日益摆脱对西方技术的依赖，并致力于发展自己的生态系统。这款笔记本电脑的发布突显了华为在Windows许可证到期后，利用鸿蒙系统为其PC产品提供动力的战略转变。
...
主要功能包括企业搜索，可安全连接到 Google Drive、Sharepoint、OneDrive、Google Calendar 和 Gmail，并且能够构建自定义连接器。它可以使用来自各种数据源、文档摘要和个人知识库的知识生成个性化回复。MCP 支持计划用于更广泛的企业系统连接。
...
SPLADE通过使用语义模型来识别文档中应该存在的关键术语（合成标记），即使这些术语没有明确出现，从而解决了这些问题。 然后，这些标记被索引到传统的搜索引擎中，从而提高召回率。
...
此次收购旨在巩固哈曼作为消费音频市场全球领导者的地位，哈曼在便携式音频设备领域已占据60%的市场份额。三星计划在其包括智能手机、电视和耳机在内的产品线中利用所收购品牌的音频技术。
...
美国国立卫生研究院官员的证词表明，政府效率部，特别是前麦肯锡顾问瑞秋·莱利，指示终止资助，提供要取消的资助名单和解释决定的预先写好的信件。这与政府声称政府效率部仅仅是一个咨询机构的说法相矛盾。文章强调了削减经费的空前规模，并质疑终止资助的科学依据。华盛顿州总检察长正在采取法律行动，指控政府藐视法庭命令并隐瞒终止资助背后的动机。政府为削减经费辩护，称这是将美国国立卫生研究院的重点重新放在关键公共卫生优先事项上所必需的。
...
## 44. Foundation DB Record Layer SQL API
基金会数据库记录层 SQL API
...
网络搜索已在 Claude Code 中提供，可通过最新的开发工具和库信息来增强开发工作流程。来自 Poe 和 Adaptive.ai 的客户聚光灯展示了该工具的成本效益、速度、彻底性和准确性。
...
## 53. Show HN: 适用于 Kindle Comic Converter 的 eInk 优化漫画 (+Kobo/ReMarkable)
...
该报告强调了网络间谍活动的不断演变以及保持警惕的重要性，特别是对于那些参与伊朗活动人士社区的人。它建议谨慎对待未经请求的联系，并独立验证网站和优惠。Palo Alto Networks的客户通过高级URL过滤、高级DNS安全和高级威胁防御受到保护。该报告最后提供了入侵指标（IOC），包括虚假网站的域名和URL，以及Unit 42事件响应的联系信息。
...
该研究表明，一个独特的北非谱系在相对孤立的环境中存在了很长一段时间，而绿色撒哈拉时期畜牧业的传播主要是一种文化现象。
...
安大略省将开建加拿大首个小型模块化反应堆，预计至2050年电力需求将增长75%。道格·福特省长政府已批准该项目，由安大略发电公司(OPG)牵头，计划建造四个小型模块化反应堆，总成本为209亿美元。
...
宾夕法尼亚州立大学的研究人员开发出一种新型毛发状电极，用于长期脑电图监测，与传统脑电图电极相比，该电极具有更高的舒适性、稳定性和信号质量。这种轻便、灵活的电极由3D打印水凝胶材料制成，使用生物粘合墨水直接粘附在头皮上，无需繁琐的凝胶和大量的皮肤准备。

这项新技术解决了传统脑电图监测的几个局限性，传统脑电图监测繁琐、耗时，并且由于电极与头皮之间的间隙或刚性电极的移动而容易导致信号衰减。该毛发状电极可在连续佩戴超过24小时的情况下保持稳定的性能，提供高质量的记录而不会降低信号质量。

这种新型电极的主要优点包括提高了患者的舒适性和隐蔽性，因为它模仿了人头发的外观，甚至可以打印成不同的颜色以匹配人的头发。此外，无需凝胶和电极的牢固粘附确保了数据的一致性，即使在不同的监测会话中也是如此。虽然目前是有线的，但未来的研究旨在开发无线版本，以提高患者的活动性。研究人员认为，这项技术在临床医疗保健应用以及消费者健康和保健产品方面具有潜力。

---

## 67. 美国国家安全局曾通过愤怒的小鸟及其他应用进行间谍活动：报告 (2014)

**原文标题**: NSA spied through Angry Birds, other apps: report (2014)

**原文链接**: [https://www.nbcnews.com/tech/tech-news/nsa-spied-through-angry-birds-other-apps-report-flna2d12006530](https://www.nbcnews.com/tech/tech-news/nsa-spied-through-angry-birds-other-apps-report-flna2d12006530)

2014年爱德华·斯诺登泄露的文件显示，美国国家安全局（NSA）和英国政府通信总部（GCHQ）利用包括热门游戏《愤怒的小鸟》在内的智能手机应用程序，在全球范围内收集用户个人数据。这些数据包括位置信息、政治倾向和性取向。情报机构能够通过谷歌地图、脸书、Flickr和Flixster等应用程序访问这些数据。

一份2012年的英国情报报告详细描述了如何从安卓设备上的《愤怒的小鸟》中提取用户信息。幻灯片显示，应用程序生成的数据可以揭示手机的设置、连接的网络、访问的网站、下载的文档以及用户的朋友。

美国国家安全局虽然没有直接评论这些报告，但表示它只收集法律授权的用于外国情报和反情报目的的通信，并且普通美国人的通信并不在其关注范围之内。英国政府通信总部也坚称其活动是经过授权的、必要的和成比例的。

该报告凸显了情报机构将看似无害的应用程序转化为间谍工具的容易程度。《纽约时报》曾短暂公布了一名参与该项目的美国国家安全局员工的名字，这一举动被批评为可能损害情报行动。

---

## 68. Unity开源双重标准：VLC禁令
...
本文详细介绍了 VideoLabs 在 Unity 应用商店的经历，重点指出 Unity 在执行其有关 LGPL 许可代码规则方面存在双重标准。作为 VLC 背后的团队，VideoLabs 的 VLC for Unity 集成资源于 2023 年被 Unity 商店禁用，理由是包含 LGPL 依赖项。

核心问题在于，虽然 Unity 以包含 LGPL 代码（特别是 LibVLC）为由禁用了 VideoLabs，但 Unity 编辑器和运行时*本身*也依赖于多个 LGPL 库。此外，Unity 商店中的许多其他资源也包含像 FFmpeg 这样的 LGPL 依赖项，这表明执行标准不一致。

被禁用后，VideoLabs 推出了自己的“VideoLabs 商店”，以继续提供 VLC for Unity 插件的二进制文件，以及 LibVLC 商业许可证、电子书和侧重于 LibVLC 和 FFmpeg 的灵活多媒体咨询服务包。这使得现有和新客户能够在 Unity 生态系统之外访问该插件。咨询服务包旨在为在其项目中使用 LibVLC 或 FFmpeg 的公司提供专业支持、定制构建和集成协助。VideoLabs 由 VLC 和 FFmpeg 专家组成，随时准备为客户提供定制解决方案。
...
**原文链接**: [https://www.stefanjudis.com/blog/stringly-typed/](https://www.stefanjudis.com/blog/stringly-typed/)

Stefan Judis 的文章《字符串类型化》探讨了在 API 调用等场景中使用字符串代替更具体类型的情况。他认为 Scott Hanselman 创造了“字符串类型化”这个术语，并将其定义为在存在更好类型时传递字符串，这在 API 交互中很常见，原因是 JSON 的类型系统有限。

尽管 JavaScript 的弱类型化历史使得这个问题不那么突出，但 Judis 认为，在 TypeScript 等强类型环境中，字符串类型化的 API 可能会出现问题，因为类型信息会丢失，并且必须手动重建。他将 API 端点重新视为远程函数调用，强调了从 API 接收诸如 `any` 或 `unknown` 等通用类型时类型安全性的丧失。

Judis 承认字符串类型化接口在单页应用程序 (SPA) 中的普遍存在，并表达了对网络端到端类型安全性的渴望。他指出，像 tRPC 和 GraphQL 这样的解决方案可以解决这个问题，暗示将探索网络上的类型安全性。他想知道他对“字符串类型化”接口的新认识是否会让他更频繁地发现和批评它们。

---

## 70. OpenAI 国家版

**原文标题**: OpenAI for Countries

**原文链接**: [https://openai.com/global-affairs/openai-for-countries/](https://openai.com/global-affairs/openai-for-countries/)
...
本文题为《多少鲜血》，调查了《经济学人》杂志提出的“血液制品占美国商品出口额的1.8%”这一论断的准确性。作者使用详细的协调关税制度（HTS）编码，对美国贸易统计数据进行了细致的分析。

作者将“人血；供治疗、预防或诊断用的动物血制品……”的HTS编码分解为各种子类别，并细致地将它们归类为：肯定含有人血的、几乎不含有人血的以及可能含有人血的。

通过这一过程，作者确定几乎肯定使用血液的商品出口额约占美国商品出口总额的0.5298%。然后，作者根据行业知识，估算“可能含血”类别中实际使用人血的产品的百分比，得出结论，另有0.1569%的出口产品包含血液。

因此，作者认为，更准确的数字约为美国商品出口额中涉及人血的0.6867%，远低于《经济学人》引用的1.8%。作者承认其中涉及粗略估算，但旨在提供更详细、更透明的数据分析，鼓励进一步的审查和修正。
...
本文详细介绍了一项关于乌贼交流的新发现：它们使用特定的手臂动作（被称为“手臂挥舞信号”）以多感官方式相互交流。研究人员识别出四种不同的手势：“向上”、“侧向”、“翻滚”和“加冕”，每种手势都涉及不同的手臂姿势、波动，并且通常伴有颜色变化。

研究发现，乌贼更容易对这些手臂挥舞信号的垂直视频做出反应，表明它们对与社会相关的展示具有复杂的理解。至关重要的是，研究表明这些信号不仅仅是视觉上的；乌贼还可以通过水中的振动感知到它们，可能利用它们的侧线和平衡囊来检测手臂运动产生的机械波。回放实验表明，乌贼对原始录音的反应比对打乱或倒放的版本更强烈。

这种双通道（视觉和振动）通信系统突出了头足类动物和脊椎动物之间的进化相似性，表明用于检测水运动的感觉系统可能已经进化为与脊椎动物听觉类似地服务于通信功能。虽然每个手臂信号的确切含义仍然未知，并且可能依赖于具体环境（交配、狩猎、防御），但这一发现扩展了我们对海洋无脊椎动物通信的理解，并强调了头足类动物的智能。研究团队希望这项工作能够为跨物种交流和环境互动提供见解。

---

## 73. Show HN: Ductape – 一次构建后端集成，随处复用

**原文标题**: Show HN: Ductape – Build back end integrations once, reuse them anywhere

**原文链接**: [https://www.ductape.app](https://www.ductape.app)

Ductape是一个全方位API市场和平台，旨在简化后端开发和集成。它允许开发者在不同环境中构建和复用后端组件，从而加快开发周期并提高弹性。

使用Ductape的主要优势包括：

*   **快速集成：** 促进与第三方API和服务快速集成，将实施时间从数周缩短至数小时。
*   **简化开发：** 能够创建可重用的API、数据库、通知和云函数组件。
*   **增强弹性：** 提供自动重试、故障转移和供应商切换以应对停机。
*   **完全可见性：** 提供跨所有环境的全面监控，以快速识别和解决问题。
*   **事件驱动特性：** 支持创建和监控具有内置错误处理功能的强大事件工作流。
*   **后台作业与函数：** 支持创建可扩展且容错的作业和无服务器函数。
*   **统一通信：** 简化跨服务的电子邮件和通知管理。

本质上，Ductape旨在通过提供一个用于管理API、事件和作业的统一平台来加速上市时间，其重点是可重用性、弹性和可观察性。一份客户评价强调了使用Ductape后开发时间的显著缩短。
...
本文记录了作者开发 BrakeBright 的历程，BrakeBright 是一种旨在提高摩托车安全性的智能刹车灯系统。 由于对骑行的热情重燃以及对强发动机制动导致追尾事故的担忧，作者试图创造一种超越仅仅依赖骑手习惯的解决方案。

BrakeBright 能够智能检测出发动机制动引起的减速并激活刹车灯，从而提醒后方车辆。 在紧急制动期间，它还会根据制动强度成比例地闪烁刹车灯。 由于对依赖基本倾斜开关的现有产品不满意，作者旨在普及自适应刹车灯技术，该技术以前仅限于高端车辆。

开发过程涉及大量的原型设计、广泛的测试（最初使用朋友的自行车，后来使用作者自己的自行车）以及持续的改进。 挑战包括振动干扰、传感器精度以及对用户友好的固件更新的需求。 作者甚至在他的摩托车前部安装了一个 LED，以可视化地监控 BrakeBright 的动作。

测试最终在苏格兰 NC500 公路上进行了一次严格的旅行。 最终产品具有防水、抗振和易于安装的特点。 作者现在正在完成软件实用程序，以允许骑手个性化和更新 BrakeBright。 第一批产品的到来是一个非常有意义的时刻，验证了作者的奉献精神和愿景。 他鼓励读者购买 BrakeBright 产品以支持他的旅程，并提供联系方式以供反馈和讨论。 目标是使摩托车骑行对每个人来说都更安全、更愉快。

---

## 75. 维基深潜 - 维基百科中的AI引导探索

**原文标题**: Wikidive – AI guided rabbitholes in Wikipedia

**原文链接**: [https://wikidive.tulv.in/](https://wikidive.tulv.in/)

Wikidive是一款人工智能引导的工具，旨在以“兔子洞”的方式探索维基百科，鼓励深入和分支研究。该网站或应用程序似乎遇到了连接问题，提示信息如“无法找到互联网”、“尝试重新连接”和“发生错误！”。

尽管存在连接问题，但其核心功能通过一系列主题得以体现。Wikidive似乎将维基百科的内容划分为以下广泛的类别：

*   **学术学科：** 认知、哲学、计算机科学、形而上学、认识论、科学、数学、政治、经济学、历史、心理学、社会学、语言学、人类学、生物学、物理学、化学、天文学、地质学。
*   **艺术与文化：** 文学、音乐、电影、艺术、符号学、时尚、食物。
*   **科技与环境：** 科技、文化、自然、宗教、地理、工程、建筑、设计。

这些类别表明，Wikidive通过提供人工智能驱动的路径，帮助用户浏览维基百科，探索特定学科领域和相关概念。“这是什么？”链接暗示着一个包含关于如何使用Wikidive的进一步解释或说明的部分。总而言之，这是一个旨在以更专注和引导的方式探索维基百科内容的平台。

---

## 76. 谎言：1996年鲍威与在线音乐发行
...
1996年9月，大卫·鲍伊在他的网站上免费发布了单曲《Telling Lies》，标志着在线音乐发行的一次早期尝试。这项由鲍伊的数字合作伙伴N2K推动，并受维珍唱片影响的举措，旨在通过直接以电子方式发行音乐来绕过唱片公司中间商。

虽然当时已存在Music Boulevard和CDnow等在线音乐零售网站，但由于带宽较低，下载和流媒体面临挑战。用户可以通过RealAudio或Shockwave以低质量流媒体播放《Telling Lies》，或使用Liquid Audio（一种旨在保护版权的新技术）下载更大的、更高质量的文件。 Liquid Audio下载是一个9.7MB的zip文件，用户需要Liquid Audio播放器软件才能播放该歌曲。这个过程缓慢而繁琐，常常导致音频不完整或质量差。

尽管存在技术困难，《Telling Lies》的发布在营销上取得了成功，N2K报告称第一周下载量达数十万次。鲍伊参与了CompuServe上的在线聊天，以宣传该发布。

尽管鲍伊承认流媒体的“质量差”和下载速度慢，但他积极看待这次实验，认识到在线音乐令人兴奋的发展潜力。 N2K的首席执行官拉里·罗森设想这标志着艺术家资助和发行自己的音乐，绕过传统唱片公司的转变的开始。虽然数字下载的电子商务尚未到位，但N2K计划在不久的将来实施它。

---

## 77. Gmail即将停止支持传入SMTP的3DES加密算法。

**原文标题**: Gmail will soon stop support for the 3DES encryption cipher for incoming SMTP

**原文链接**: [https://workspaceupdates.googleblog.com/2025/05/update-for-gmail-support-for-the-3des-encryption-cipher-for-incoming-smtp-connections.html](https://workspaceupdates.googleblog.com/2025/05/update-for-gmail-support-for-the-3des-encryption-cipher-for-incoming-smtp-connections.html)

Gmail 将于 2025 年 5 月 30 日起停止支持传入 SMTP 连接的 3DES 加密算法。此更改旨在通过淘汰过时的加密方法来提高安全性。在此日期之后，依赖 3DES 的电子邮件系统将无法向 Gmail 帐户传递电子邮件。

收到有关 3DES 使用警告的管理员和用户应确保其电子邮件发送系统配置为使用更现代和安全的 TLS 密码。Google 在其帮助中心提供支持的密码列表。最近使用 3DES 发送电子邮件的管理员也收到了电子邮件通知。此更改影响所有 Google Workspace 客户。用户可以查阅 Google Workspace 管理员帮助，了解有关 Gmail SMTP TLS 连接支持的密码以及如何检查其电子邮件流量中 3DES 使用情况的更多信息。

---

## 78. 使用Grasshopper的参数化建模

**原文标题**: Parametric Modeling with Grasshopper

**原文链接**: [https://baharmon.github.io/basics](https://baharmon.github.io/basics)

本文介绍了使用 Grasshopper 进行参数化建模，Grasshopper 是 Rhino 的一个可视化编程界面。 它涵盖了通过算法图创建几何体的基础知识，例如点、线、折线、曲线和曲面。

文章首先解释了可视化编程的概念，强调 Grasshopper 如何使用将数据连接到函数的图表来生成复杂的形状和设计方案。 它重点介绍了学习 Grasshopper 的关键资源，包括在线入门教程和教程。

教程随后详细介绍了如何创建基本的几何元素：

*   **点：** 由 x、y 和 z 坐标定义，可以使用带有数字滑块或文本面板的“Construct Point”组件构建，或从 Rhino 中绘制的点引用。
*   **线：** 由起点和终点定义，由具有方向和长度的起点（“Line SDL”组件）定义，或从 Rhino 中绘制的线引用。
*   **折线：** 使用“Polyline”组件连接一系列点创建。 它们可以闭合以形成多边形。
*   **曲线：** 使用“Interpolate”组件通过控制点插值的 NURBS 曲线。 点可以从各种来源导出，包括正弦波。
*   **曲面：** 通过二维控制点网格插值的 NURBS 曲面。 曲面可以从原始形状、平面曲线（“Boundary Surfaces”）创建，或者通过放样（“Loft”）和直纹（“Ruled Surface”）多个曲线创建。

文章最后提到了拉伸作为创建实体的方法，并暗示了后续教程将重点介绍将这些概念应用于家具设计。

---

## 79. Polycompiler：将Python和JavaScript代码合并成一个可在两者中运行的文件
...
Polycompiler：一个将 Python 和 JavaScript 代码合并到单个文件中的实验性工具，该文件根据所用解释器执行不同的代码。使用 Node.js 运行时，执行 JavaScript 代码；使用 Python 运行时，执行 Python 代码。

其基本原理是利用 JavaScript 注释和带标签的语句，以及 Python 的 `exec` 和 JavaScript 的 `eval` 函数。Python 将 JavaScript 行解释为包含 eval 语句的 lambda 函数，但仅执行第 1 行注释中的 Python 代码。另一方面，由于注释，JavaScript 将 Python 行视为整数 (1)，然后使用标签 `lambda:` 忽略 Python 语句的开头，从而允许 `eval` 执行 JavaScript 代码。

要使用 Polycompiler，通过 `npm i polycompiler` 安装它，然后使用 `polycompiler in.js in.py out.py.js` 运行它，指定输入的 JS 文件、输入的 Python 文件和所需的输出文件名（由于 Node.js 的要求，必须以 .js 结尾）。

作者创建 Polycompiler “纯属娱乐”，并认为它可以作为一种解决方案，用单个文件同时面向 Python 和 JS 用户，即使没有安装其中一种语言。本文解释了这种多语言文件的运作方式，并鼓励读者观看作者的 YouTube 视频，以更深入地了解开发过程。
...
本文探讨了 TypeScript 类型系统中严格性和可用性之间的权衡，并引入了“过度类型化”的概念。过度类型化是指库为了追求完美的类型安全而创建过于复杂的类型，导致难以理解的定义、晦涩的错误信息，以及最终导致不安全的变通方法，例如强制转换为 `any`。

作者以 TanStack Form 为例，强调了其令人印象深刻的类型安全特性，但也批评了其类型定义的极端复杂性，例如具有 17 个泛型参数的 `FieldMeta` 类型。

文章认为，虽然严格的类型是可取的，但过于复杂的类型会阻碍开发人员的体验，使代码更难理解和维护。作者提倡使用更简单的类型，即使它们不太准确，因为它们通常会带来更实用和令人愉快的工作流程。

最后，文章提出了一种替代方法：从模式定义生成类型。Astro 框架就是一个例子，这种方法提供了更令人愉快和可维护的开发体验。

---

## 81. 无限上下文LLM的人类式情景记忆

**原文标题**: Human-Like Episodic Memory for Infinite Context LLMs

**原文链接**: [https://arxiv.org/abs/2407.09450](https://arxiv.org/abs/2407.09450)

这篇arXiv文章介绍了EM-LLM，一种旨在提升大型语言模型（LLMs）处理和保持长程上下文连贯性的新型方法，有效赋予它们“无限上下文”能力。受人类情景记忆启发，EM-LLM解决了当前LLMs在处理长序列和保持准确性方面的局限性。

EM-LLM的工作原理是使用贝叶斯惊奇度和图论边界细化，将token序列组织成连贯的“情景事件”。这种分割是实时进行的。当需要信息时，EM-LLM利用一个两阶段检索过程，结合基于相似性的检索和时间上连续的检索，模仿人类访问相关信息的方式。这允许从非常长的上下文中进行高效检索。

作者证明，即使没有经过微调，EM-LLM在LongBench和InfiniteBench等基准测试中也优于最先进的检索模型（如InfLLM），并且可以与各种基线LLM协同工作。它还超越了使用类似资源的传统检索增强生成（RAG）系统。值得注意的是，EM-LLM在许多任务中甚至超过了全上下文模型的性能，并且可以处理跨越1000万token的检索——这对于全上下文模型来说是一个计算上具有挑战性的规模。

此外，该研究揭示了EM-LLM的事件分割与人类感知到的事件之间的相关性，表明它与生物记忆机制之间存在联系，并为进一步研究人类记忆提供了一个框架。EM-LLM的关键优势在于其能够以计算效率处理实际上无限的上下文长度，并在长上下文任务中提高性能。

---

## 82. 贝尔1881年金属探测器

**原文标题**: Bell's 1881 Metal detector

**原文链接**: [http://www.scitechantiques.com/belldiscovery/](http://www.scitechantiques.com/belldiscovery/)

本文详述了亚历山大·格拉汉姆·贝尔于1881年使用早期金属探测器寻找刺杀加菲尔德总统的子弹的尝试。文章重点介绍了为探索频道节目“白宫法医”制作的道具，该节目重新审视了贝尔的努力。

文章着重介绍了贝尔使用的两种主要金属探测器设计。第一种采用了四线圈桥式方法，使用初级和次级线圈设置进行扫描，与另一组线圈平衡。这种设计的范围有限（一两英寸）。文章描述了贝尔在白宫使用该设计的困境，包括接线错误和来自断续器（蜂鸣器）的干扰。

第二种设计基于贝尔的电话工作，采用了两个大型线圈（初级和次级）。这种设计更灵敏。文章描述了贝尔和他的助手如何匆忙制造这种新型探测器，并在总统身上进行了测试，将设备放置在另一个房间以减少噪音。然而，尽管贝尔做出了努力，但他仍然无法明确地确定子弹的位置，加菲尔德后来死于感染。

文章还提供了有关两种探测器设计中使用的电路的详细信息，并包括探索频道节目拍摄幕后的照片，展示了演员和道具再现贝尔的实验室。最后，它宣传了探索频道节目中使用的科学仪器道具的销售。

---

## 83. Gemini 2.5 Pro 预览
...
以下是Gemini 2.5 Pro预览版文章的简要总结：

谷歌在Google I/O大会之前发布了更新版的Gemini 2.5 Pro，称为I/O版。新版本显著提升了编码能力，尤其是在前端和UI开发方面。它还增强了代码转换、编辑和创建代理工作流等基本编码任务。

据Replit称，Gemini 2.5 Pro提供了最佳的“能力/延迟”比率。Cognition强调了其处理复杂代码重构任务的能力，堪比高级开发人员。该模型在WebDev Arena排行榜上排名第一，可以构建美观且功能齐全的Web应用程序，为Cursor中的创新代码代理以及与Cognition和Replit的协作提供支持。

Gemini 2.5 Pro的视频理解能力有所提高，在VideoMME基准测试中达到84.8%。这使得新的应用成为可能，例如从YouTube视频生成交互式学习应用程序，正如在Google AI Studio中演示的那样。该模型擅长更轻松的特性开发，允许开发人员在IDE中快速复制UI样式并为新特性生成代码。文章还展示了一个听写入门应用程序，作为其美学Web开发能力的示例。

开发人员可以通过Google AI Studio中的Gemini API访问Gemini 2.5 Pro，而企业客户可以在Vertex AI上使用它。现有的Gemini 2.5 Pro用户将自动受益于更新后的模型，无需进行任何更改。谷歌还更新了带有新版2.5 Pro的模型卡。

---

## 84. 我付了整个GPU的钱，我就要用整个GPU。

**原文标题**: 'I paid for the whole GPU, I am going to use the whole GPU'

**原文链接**: [https://modal.com/blog/gpu-utilization-guide](https://modal.com/blog/gpu-utilization-guide)

查尔斯·弗莱的这篇文章深入探讨了“GPU利用率”这一多面概念，将其分解为三个不同的类别：GPU分配利用率、GPU内核利用率和模型FLOP/s利用率（MFU）。

**GPU分配利用率**衡量的是实际运行应用程序代码的GPU秒数与支付的总GPU秒数之比。它受到经济因素（如合同承诺）和DevOps限制（如启动时间慢）的影响。Modal旨在通过聚合GPU供需，以及通过优化其容器堆栈以实现更快的部署来改善这一点。

**GPU内核利用率**表示GPU实际执行代码（特别是CUDA内核）的时间比例。低内核利用率可能源于不利用GPU的任务（如数据传输），或者CPU未能足够快地向GPU提供工作（“主机开销”）。CUDA图和批处理等技术可以缓解这种情况。

**模型FLOP/s利用率 (MFU)** 是最深层的利用率，它将实现的FLOP/s吞吐量与理论FLOP/s带宽进行比较。低分配或内核利用率必然会导致低MFU。优化通信、解决内存瓶颈（尤其与低算术强度工作负载（如基础模型推理）相关），以及认真编写高效的GPU内核对于最大化MFU至关重要。

本质上，这篇文章认为真正的GPU利用率不仅仅是拥有已分配的GPU，还需要一种整体方法，解决从分配效率到内核优化再到内存带宽考虑的整个堆栈。

---

## 85. Stripe稳定币金融账户

**原文标题**: Stripe Stablecoin Financial Accounts

**原文链接**: [https://docs.stripe.com/crypto/stablecoin-financial-accounts](https://docs.stripe.com/crypto/stablecoin-financial-accounts)

Stripe稳定币金融账户（目前为私测预览版）允许100多个国家/地区的企业使用稳定币管理资金。企业可以使用法币或加密货币渠道接收、存储和发送美元和稳定币资金。支持的地区包括美洲、欧洲、非洲和亚洲的广泛国家/地区。

要开始使用，用户可以使用银行转账（ACH或电汇）或从外部钱包转入加密货币，在Stripe Dashboard中为其稳定币余额充值。然后，用户可以共享账户详细信息以接收资金。Stripe旗下公司Bridge担任稳定币余额的托管人，可能会将资金保存在USDC（Circle）或USDB（Bridge）中。USDB是一种封闭式稳定币，由美元1:1支持。

用户可以将资金从其稳定币余额转移到外部银行账户或加密货币钱包，非加密货币转账会自动进行汇率转换。他们还可以通过ACH或电汇以美元或通过加密货币渠道以稳定币向他人汇款。收款人可以是现有或新的，可以在Directory页面上配置支付方式。

主要考虑因素包括Bridge担任托管人，以及Stripe将余额保存在USDC或USDB中。USDB由现金和贝莱德的短期货币市场基金中持有的美元1:1支持。

---

## 86. 迷走神经刺激消除创伤后应激障碍：研究

**原文标题**: Vagus Nerve Stimulation Erases PTSD: Study

**原文链接**: [https://neurosciencenews.com/vagus-nerve-stimulation-ptsd-28818/](https://neurosciencenews.com/vagus-nerve-stimulation-ptsd-28818/)

一项突破性的一期临床研究报告：迷走神经刺激（VNS）联合延长暴露疗法有望消除对抗传统治疗有抵抗性的PTSD患者的病情。该研究由UT Dallas和贝勒大学医学中心的研究人员进行，共有九名对抗治疗有抵抗性的PTSD参与者接受了12次延长暴露疗法，并结合通过植入设备进行的短时VNS刺激。

值得注意的是，所有参与者都完全缓解了PTSD，不再符合诊断标准，并且PTSD症状和严重程度显著改善，这些改善至少在治疗后持续了六个月。这种100%的缓解率尤其值得关注，因为许多PTSD患者对标准疗法没有反应，并且通常终生保持诊断。

该研究利用了VNS已确定的增强神经可塑性的能力，促进大脑重塑，并改善对传统治疗无反应的个体的情况。下一步是进行一项双盲、安慰剂对照的二期临床试验，以验证这些发现并寻求FDA批准。研究人员强调，这种方法有望为患有PTSD的人们带来新的希望，这些PTSD可能源于各种创伤经历，并强调了临床前和临床团队之间研究的合作性质。他们开发了一种新型VNS设备，比以前的型号更小、更便宜。

---

## 87. DoorDash 将收购 Deliveroo
...
DoorDash拟以39亿美元收购英国外卖公司Deliveroo，标志着DoorDash继2022年收购Wolt后，进一步扩张其国际业务。该交易对Deliveroo的估值为每股180便士（溢价44%），已获得Deliveroo董事会的批准。

此次收购标志着Deliveroo一段艰难时期的结束，该公司于2021年进行了糟糕的首次公开募股，股价大幅低于发行价。投资者对新冠疫情后增长的可持续性、激烈的市场竞争以及与零工经济相关的法律挑战的担忧，导致了股价下跌。

DoorDash首席执行官Tony Xu对此次收购表示兴奋，强调了两家公司的合并覆盖范围，涵盖超过40个国家，人口超过10亿。此次收购反映了外卖行业的持续整合，其他近期交易包括Deliveroo出售其香港业务以及Just Eat被Prosus收购。

---

## 88. 对齐并非免费：模型升级如何让你的置信度信号失效
...
此方差文章《对齐并非免费：模型升级如何扼杀你的置信度信号》强调了一个持续升级AI模型的潜在问题：**升级可能会对模型生成的置信度评分的可靠性产生负面影响。**

核心论点是，虽然升级通常被认为有利于提高准确性和能力，但模型对齐（使其遵守特定指南和安全措施）的过程可能会无意中影响其置信度评分的校准。 具体而言，用于防止模型表达不良输出的技术也可能会抑制其准确表达不确定性的能力。

文章表明，置信度信号不可靠的模型会构成重大风险。 用户可能会高估模型的能力，从而导致不适当的依赖，并可能在关键应用中造成有害结果。

因此，核心要点是呼吁谨慎。 组织需要仔细考虑模型对齐与升级过程中置信度评分完整性之间的潜在权衡。 每次升级后监测和验证置信度评分的校准变得至关重要，以确保模型的自我评估能力不受损害。 文章含蓄地建议探索可以最大限度地减少对置信度评分影响的替代对齐技术，或者实施在对齐后重新校准模型的策略。
...
本文批判了一种常见的生成0到1之间均匀随机浮点数的方法（生成随机整数、转换为浮点数并除法），因为它对可能的浮点数完整范围的可访问性有限，并且在最低有效位存在偏差。文章提出了一种新的“完美”随机浮点数生成算法，以最小的性能损失解决了这些缺陷。

作者解释了浮点数的结构（符号、指数、尾数）及其幂律分布，强调精度在不同量级上是恒定的。他们讨论了浮点数舍入模式及其对随机数生成的影响，特别是在指数边界上的概率。

核心思想是基于将实数舍入到该浮点数的可能性，匹配生成每个浮点数的概率。所提出的两步算法包括：1）生成一个定点随机数来确定从中抽取最终值的实数范围，并递归地处理较低的数字范围；2）通过回填可用的额外精度位来完成数字。第二阶段根据选择的舍入模式而变化。文章包含了实现该算法的Go代码，详细介绍了缩放定点阶段和最终化过程。这种方法确保了在浮点数范围内实现真正的均匀分布。
...
VVVVVV (2010) 独立游戏源代码发布公告。该游戏由 Terry Cavanagh 创作，Magnus Pålsson 配乐。桌面版源代码可供个人编译使用。仍鼓励商业购买以支持开发者。已编译版本的发布受 LICENSE.md 文件约束。关于更新的讨论请前往非官方 VVVVVV Discord 服务器的 vvvvvv-code 频道。

鸣谢：Terry Cavanagh (创作者), Bennett Foddy (房间名称), Magnus Pålsson (音乐), FamilyJules (金属原声), Simon Roth (2.0 版本更新 - C++ 移植), Ethan Lee (2.2 版本更新 - SDL2/PhysicsFS/Steamworks 移植), Misa Kai (额外代码)。Beta 测试：Sam Kaplan 和 Pauli Kohberger。结尾图片：Pauli Kohberger。游戏本地化由多个团队完成，GitHub 上也有许多其他贡献者。

---

## 91. 面向Astro开发者的RSC

**原文标题**: RSC for Astro Developers

**原文链接**: [https://overreacted.io/rsc-for-astro-developers/](https://overreacted.io/rsc-for-astro-developers/)

本文面向 Astro 开发者解释 React Server Components (RSC)，着重强调它们的异同。Astro 和 RSC 均将服务端逻辑与客户端交互分离，Astro 使用 `.astro` 组件和客户端岛屿，而 RSC 使用 Server Components 和 Client Components，并通过 `'use client'` 指令区分。

主要区别在于灵活性和集成程度。Astro 在服务器和客户端代码之间提供了清晰的视觉区分，但可能导致在共享 UI 组件和管理嵌套组件内的交互行为方面存在局限性。另一方面，RSC 在两侧都使用 React，从而可以实现更高的代码重用率和统一的 React 树，该树集成了诸如上下文共享、类似 SPA 的导航和就地服务器端刷新等功能。

虽然 RSC 提供了更高的灵活性并解决了 Astro 的一些限制，但它也带来了更陡峭的学习曲线。作者指出，RSC 的开发者体验仍然“粗糙”，但建议学习它，因为它包含有趣的想法以及解决问题的潜力。对于那些难以掌握 RSC 的人来说，Astro 也可以作为通往类似概念的更温和的入门方式，并且可以为客户端 React 开发者解决他们可能没有意识到的问题。最终，RSC 被定位为框架的构建块，而 Astro 则是一个成熟的框架。

---

## 92. Nnd – GDB、LLDB 的 TUI 调试器替代品

**原文标题**: Nnd – a TUI debugger alternative to GDB, LLDB

**原文链接**: [https://github.com/al13n321/nnd](https://github.com/al13n321/nnd)

`Nnd`：一款快速的Linux TUI调试器，旨在成为GDB和LLDB的快速替代品。它从头开始实现，不依赖现有调试器。其主要卖点是速度，通过多线程和带进度条的异步进程，尽可能实现即时操作。

然而，它也有局限性：仅支持Linux/x86-64，仅支持原生代码（C++/Rust），仅支持TUI，仅支持单进程，并且没有记录/回放功能。它包含常见的调试功能，如断点、单步执行、代码/反汇编视图、监视表达式以及C++和Rust标准库的美化打印。自动向下转型抽象类是一个显著的特性。

该调试器已被作者积极使用并认为很有帮助，但尚未经过广泛测试，并且可能缺少功能。作者建议探索界面并使用`--help-*`获取文档，并计划制作教程视频。

`Nnd`作为一个6MB的单文件可执行文件分发，没有依赖项，可以通过`curl`和`chmod`安装。从源代码构建需要Rust、musl目标和musl-tools。作者鼓励提供反馈，并承认在可发现性方面的局限性。

---

## 93. 展示一下：Clippy – 为本地LLM设计的90年代UI
...
“Clippy” 是一款桌面应用程序，提供 90 年代风格的用户界面，用于与本地大型语言模型 (LLM) 交互。这是一个怀旧的艺术项目，向微软的 Clippy 以及那个时代的设计美学致敬。

该应用程序允许用户直接在他们的计算机上与 LLM 聊天，无需复杂的设置。它利用 llama.cpp 和 node-llama-cpp 根据可用硬件（Metal、CUDA、Vulkan 等）自动优化模型性能。用户还可以加载自定义模型、提示并调整参数。

Clippy 强调离线和本地运行，确保隐私和免费使用。唯一的网络活动是可选的更新检查。该项目是开源的，并提供 macOS (Apple Silicon & Intel)、Windows 和 Linux (RPM & Debian) 的下载链接。

创作者感谢微软对 Clippy 原始设计和对 Electron 的贡献。他还感谢其他几位开发者对该项目的贡献。创作者将其定位为一个有趣的混合项目，将对 90 年代技术的怀旧与尖端的 LLM 功能相结合。

---

## 94. 潜在空间中的口音：AI 如何听懂英语口音强度
...
本文探讨了人工智能，特别是BoldVoice的机器学习模型，如何理解和量化英语语音中的口音强度。其核心概念是“口音指纹”，这是一种从语音录音中生成的嵌入，代表了个人的口音。

本文将这些指纹在潜在空间中可视化，其中接近度反映了口音的相似性。通过分析潜在空间，作者发现口音强度与说话者的母语无关。

本文通过跟踪一位非英语母语者Victor练习模仿美式英语口音的进度，展示了该模型的实际应用。他们使用了诸如降噪和口音转换（将Eliza的口音映射到Victor的声音上）等技术来辅助Victor的练习。潜在空间可视化有效地展示了Victor向“目标”口音的进步。

主要发现包括该模型能够准确区分口音强度，该模型没有对不同母语产生偏见，口音强度可以通过练习来提高，语音转换是口音训练的有用工具，以及降噪对口音强度测量没有显著影响。

本文最后提出了口音强度指标的应用，例如跟踪学习者的进步，评估自动语音识别（ASR）系统在不同口音下的表现，以及监控文本转语音（TTS）系统是否存在“口音漂移”。他们宣布未来的文章将进一步探讨英语中的口音指纹。

---

## 95. 关于莱姆病治疗和持续症状的新研究提供了见解
...
题为“新研究深入了解莱姆病的治疗和持续症状”的文章（2025年5月2日发表）侧重于莱姆病相关的最新研究。虽然全文不可用，但标题表明这些研究阐明了两个关键领域：

1.  **莱姆病的治疗：** 文章可能讨论莱姆病治疗方法的进展。这可能包括新药、疗法或更有效的使用现有疗法的方式。

2.  **持续症状：** 文章可能探讨一些人在完成标准莱姆病治疗后仍然经历的持续或慢性症状。这些通常被称为治疗后莱姆病综合征 (PTLDS) 或慢性莱姆病。这些研究可能调查这些持续症状的原因以及管理或缓解它们的潜在策略。

文章第二行引用的“机器人触摸传感器”与莱姆病的主题无关。该文章是在报告这些研究。

---

## 96. 哪些国家生活水平最高，哪些国家最低？

**原文标题**: Which countries have the best, and worst, living standards?

**原文链接**: [https://www.economist.com/graphic-detail/2025/05/06/which-countries-have-the-best-and-worst-living-standards](https://www.economist.com/graphic-detail/2025/05/06/which-countries-have-the-best-and-worst-living-standards)

发表于2025年5月的这篇文章探讨了193个国家的人类发展状况，并强调了一个令人担忧的趋势：新冠疫情期间生活水平的倒退可能是长期性的。尽管许多人希望迅速复苏，但五年后的数据显示，全球人类发展持续停滞的可能性更大。文章暗示，疫情对医疗、教育和经济的影响对全球生活水平产生了重大而持久的影响。
...
DJ·布雷肯讲述了他从对学校午餐债务一无所知到成为犹他州消除午餐债务倡导者的历程。在得知孩子们因无力支付午餐而面临的羞辱后，他深受触动，决定采取行动，并支付了布拉夫代尔小学835美元的午餐债务。

这一举动促使他调查更大的问题，发现犹他州学校有280万美元的午餐债务。他开始制作电子表格，并联系学区以了解更多信息，意识到这个问题不仅影响低收入家庭，也影响那些仅仅超过免费午餐计划资格标准的家庭。

布雷肯创立了犹他州午餐债务救济基金会，通过社交媒体筹集了超过5万美元，用于消除12所学校的午餐债务。他的工作使他与校长、记者和州议员接触，使他成为该问题的意外专家。他纠结于一个哲学难题：偿还债务是否会助长一个破损的体系。

尽管存在矛盾，布雷肯仍倡导立即提供救济和进行系统性变革。他的工作促成了HB100法案的通过，该法案为低价午餐学生提供免费午餐，并禁止午餐羞辱行为。他强调在解决眼前需求的同时努力寻求长期解决方案的重要性，并设想一个孩子们可以简单地吃饭而没有经济障碍的世界。
...
Anukari，一款用于音频生成的3D物理合成器，依靠GPU进行实时模拟。开发者Anukari正在向苹果公司呼吁，因为macOS的电源管理系统阻碍了Anukari在Apple Silicon设备上的性能。 macOS在检测到处理需求较低时会降低GPU时钟频率，但Anukari短暂的、突发式的GPU使用无法触发必要的时钟速度提升，以满足实时音频的要求。

虽然Anukari通过“浪费加速”的变通方法（一个人为加载GPU的自旋循环）对大多数macOS用户来说运行良好，但一些用户，特别是那些拥有Pro或Max Apple硬件的用户，会遇到性能问题。 Anukari推测，多个GPU芯片组的独立时钟控制可能将自旋循环分配给了一个快速时钟GPU，而音频工作负载却分配给了一个慢速时钟GPU。

Anukari提出了潜在的解决方案：将音频工作组的概念扩展到GPU，在Metal API中为MTLCommandQueue提供一个实时灵敏度选项，或者找到他尚未发现的现有解决方案。游戏模式不适用，因为Anukari作为插件运行，并非全屏。 由于延迟敏感性，流水线不适用，并且在同一队列中运行自旋内核可能会干扰物理内核。内核对冲被认为过于昂贵和浪费。 尽管经过彻底优化，包括FP16数学和指令重排序，但核心问题仍然在于GPU时钟管理。 他强调了讽刺的一点，即具有较弱GPU的Windows机器由于更多的用户控制或更少的激进电源管理而表现更好。

---

## 99. 使用 Rust 编写的 POSIX 和 Bash 兼容的 Shell：Brush (Bo(u)rn(e) RUsty SHell)
...
Brush (Bo(u)rn(e) RUsty SHell) 是一个用 Rust 编写的、兼容 POSIX 和 Bash 的 Shell，专为交互式使用和脚本执行而设计。 它构建并测试于 Linux 和 macOS 上，并提供实验性的 Windows 支持。 虽然它可以作为日常使用并能够运行大多数 sh/bash 脚本，但该项目建议在生产环境中谨慎使用，因为可能存在行为差异。 欢迎贡献和反馈。

用户可以通过 `cargo install --locked brush-shell` 安装 Brush，或使用 `cargo run` 直接从克隆的存储库运行它。 Nix 和 Arch Linux 用户有其他安装方法。 该 Shell 处理标准配置文件（如 `.bashrc`），并允许使用 `~/.brushrc` 文件进行自定义外观。

已知限制包括 `set` 和 `shopt` 选项的未完成实现，以及标有 `TODO` 注释或使用 `error::unimp()` 的功能。 测试包括将 Brush 的行为与其他 Shell（即 “oracle”）进行比较，超过 550 个集成测试评估标准输出和退出代码。 该项目利用了 `reedline`、`clap`、`fancy-regex`、`tokio` 和 `nix` 等 crate，以及 `pprof-rs` 和 `criterion.rs` 等工具进行测试和性能分析。 Brush 从其他非 C/C++ Shell（如 Nushell、rusty_bash 和 mvdan/sh）中汲取灵感。
...
本文介绍“Old Timey Mono”和“Old Timey Code”两种等宽字体，旨在唤起人们对20世纪早期打字机美学的回忆。“Old Timey Mono”的灵感来源于为小字号印刷设计的可读性字体“Reproducing Typewriter”，力求成为一种具有复古感的编程字体。“Old Timey Code”是专门为编程定制的变体，具有带斜线的零以及其他为清晰度而调整的字符。

该字体设计为12pt的Pica尺寸，使其适合剧本写作。它拥有广泛的字符集覆盖，包括ASCII、ISO-8859-1、Code Page 437/850、WGL4、AGLFN、Windows 1252、Mac OS Roman（不包括连字）以及多个Unicode块。它支持拉丁文、西里尔文和希腊文等多种语言，详细信息请参见语言覆盖检查器。

作者承认对原始字体进行了细微修改，以实现真正的等宽设计。他还为对打字机风格字体感兴趣的用户推荐了类似的免费和付费字体，以及他创建的其他字体。这些字体采用SIL OFL 1.1许可。文档开头醒目地放置了从Releases下载字体的链接以及字体样本的链接。
